...
const populateWithAgencyCSV = async () => {
  ...
  // Extract fields and insert into the appropriate tables
  for (let i = 1; i < rows.length - 2; i++) {
    ...
    // Loop through the ORI codes and insert into ORICode table
    for (const ORICode of ORICodeList) {
      await insertIntoORICodeTable(agencyID, ORICode);
    }
  }
...
const populateWithIncidentCSV = async () => {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const state = rowData[columnNameIndices.state].replace(/"/g, '');
    // CityID
    const cityID = await insertIntoCityTable(cityName, county, state);
